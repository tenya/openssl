...
void getsockinfo(SOCKET fd)
{
	int sndbuf = 50;
	int length = sizeof sndbuf;
	if(setsockopt(fd,SOL_SOCKET,SO_SNDBUF,(char*)&sndbuf,length) == SOCKET_ERROR)
	{
		fprintf(stderr,"getsockopt() failed: %d\n",WSAGetLastError());
		WSACleanup();	
		return ;
	}
	if(getsockopt(fd,SOL_SOCKET,SO_SNDBUF,(char*)&sndbuf,&length) == SOCKET_ERROR)
	{
		fprintf(stderr,"getsockopt() failed: %d\n",WSAGetLastError());
		WSACleanup();	
		return ;
...
int main(int argc, char **argv) {
	...
	//
	getsockinfo(conn_socket);
	loopcount =0;
	while(1) {
		memset(Buffer,0,sizeof(Buffer));
		sp = sprintf_s(Buffer,sizeof Buffer,"This is a small test message [number %d]",loopcount++);
		retval = send(conn_socket,Buffer,sp + 1,0);
